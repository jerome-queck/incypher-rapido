...
- Keep dynamic local analysis in the normal queue. Park instance-ready work outside the five challenge slots; wake it with priority only when the single Board lease and one challenge slot are free. One challenge owns at most one instance shared by all its peers; prove deletion before transfer.
...
- Leave `RAPIDO_WATCH_BOARD=true` for competition runs. After work drains, poll cheaply until the original deadline; resume new or materially changed challenges without resetting the deadline. Full refreshes compare bounded attachment bytes; old-generation facts remain audit history but cannot drive new-material work. Use `rapido monitor --follow --interval 10 --record-jsonl` for sanitized queue and cgroup evidence.
