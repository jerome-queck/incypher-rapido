...
- For non-scored agent work, whenever an instruction selects `gpt-6-astra` and that selection cannot start or operate, retry the same work with `gpt-daybreak-blue-latest` at `xhigh`. For any other unavailable model, always apply that lane's explicitly allowed fallback; if none is specified, use `gpt-daybreak-blue-latest` at `xhigh`. Record the requested/effective descriptor and reason; never fall back silently.
...
- Run every Codex subagent as Daybreak Luna (`gpt-5.6-luna`) at `xhigh` or `max`, never an unqualified/default Luna selection. Prefer `xhigh`; use `max` for the hardest bounded implementation or review. If the requested Luna effort cannot start, retry the same lane once at the other allowed Luna effort and record requested/effective descriptors plus the reason. If neither works, record failure and return the lane to the main session. Do not route Codex subagent work to another model.
...
- An operator stop must first persist the supervisor `stopping` fence. A restart from that phase may retry only Run interruption; it must never launch a worker or settle pending/unread effects. Use a 190-second outer stop grace for the 180-second worker drain and cleanup/persistence tail.
...
- Keep five challenge residencies full while five unresolved challenges exist. First-pass coverage outranks every retry: a dynamic local-to-target transition yields while any catalogue item has neither started episode zero nor terminally settled before solver start. A capacity-deferred initial dynamic follows other untouched first passes while retaining initial status. If an untouched first pass lacks the 600-second admission floor, close admission instead of spending the remainder on a shorter retry. After first-pass coverage, an instance-ready challenge blocked behind the occupied lease yields while productive local work is queued; otherwise it stays resident. One challenge owns at most one instance shared by all its peers; one target wave ends the lease, and deletion is proved before transfer.
...
- Report Board submission outcomes, correct-candidate provenance, independent verification, and scheduler lifecycle separately. The legacy `solved` count is not a benchmark score.
- Treat generic `already_solved` as a settled Board effect, not a validated candidate. Keep the private candidate unresolved and queued for a fresh same-run Verifier without resubmitting it; close only after independent verification.
...
- On 2026-09-18 the owner moved issue #19's 19,800-second acceptance run to a different place. This
  task closes after confirmed last-run audit fixes, independent review/green merges, and a fresh
  1,800-second unattended diagnostic with clean lifecycle and cleanup. Record this as a
  post-observation amendment; never claim the original 19,800-second gate passed.
